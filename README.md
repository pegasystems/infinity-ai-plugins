...
To test a local `infinity-skills` checkout with Claude Code, GitHub Copilot CLI, or Codex, set `PEGA_LOCAL_SKILLS_PATH` to the directory that contains `manifest.json` before starting the client.

```bash
export PEGA_LOCAL_SKILLS_PATH=/path/to/infinity-skills
```

If the variable is unset or empty, the plugin uses the normal bundled or cached skills payload.
