...
`pyFlowType` is the flow identity key (with `pyClassName`). `pyWorkClass` must
always equal `pyClassName`. These are enforced by the server and will cause
save failures if mismatched.

### Flow naming

Flow names (`pyFlowType`) cannot contain hyphens (`-`).
...
If the current flow has `pyCategory: "ScreenFlow"`, Approval shapes are not supported because ScreenFlow flows can only call other ScreenFlows as subprocesses.
...
| `pyFlowType` | self-register flow name | flow name or `"FlowStandard"` | flow name or taxonomy | `"FlowStandard"` (decoupled) | self-rule wins over `"ScreenFlowStandard"` default | `"FlowStandard"` | `"FlowStandard"` (decoupled) | `"FlowStandard"` |
