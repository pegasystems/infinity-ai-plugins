...
`pyKeyDefList`) and the value the agent must supply.
...
To persist a savable Data Page (`pyPageType: "savable"`), use the
`Save-DataPage` activity method with `DataPage`, a key property (e.g.
`pyGUID`), and `WriteNow: "true"`. This works for both local database
...
**Verifying a savable Data Page's save plan from outside Infinity Studio:**

- **Infinity Studio's Data Designer UI** — open a list View with "Allow
  edit, delete, and create" enabled. This is the primary supported path;
  see `methodology-integration`'s "Key Constraints" section for
  why this requirement exists.
- **The DX API data-object-action endpoint** —
  `GET`/`PATCH /data/{dataObjectName}/actions/{actionID}` (see the
  `pega-dx-api-endpoints` skill's endpoint table, row "Read or edit a data
  object"). No tool in `infinity-rules-mcp` currently exposes this
  endpoint — the MCP server's tools cover cases, assignments, data views,
  and rules, but not data-object actions. This means an MCP-based agent
  currently cannot verify a savable Data Page's connector-based write-back
  (PATCH/DELETE/CREATE save options) purely through its own tool calls.
  This is a real capability gap — flag it to the user rather than implying
  a workaround exists.
