...
Use exactly one implementation based on the resolved display type. Do not include both
implementations in the same script because the controls are mutually exclusive.

For a `Drop-down list`:

```typescript
if (params["<userRef>"]) { await page.getByTestId('<label>:select:control').selectOption(params["<userRef>"]); }
```
For a `Search box`:
```typescript
if (params["<userRef>"]) { await commonUtils.Handle_ReferenceListMethods(page, 'autocomplete', '<Label>', 'singleselect', params["<userRef>"]); }
```

### Single reference (case or data)

`params["LABEL"]` uses `LABEL:ID`; the label drives UI selection and the ID drives DX API
mapping.

```typescript
if (params["LABEL"]) { await commonUtils.Handle_ReferenceListMethods(page, 'DISPLAY_AS', 'LABEL', 'singleselect', params["LABEL"]); }
```

If `displayAs` is `advancedSearch`, use `Handle_SearchAndSelectSingle` from `business-action-advanced-search`; for all other display types, keep using the existing `Handle_ReferenceListMethods` pattern above.

- For single-select advancedSearch, `params["LABEL"]` is the `LABEL:ID` value (e.g., `'John Smith:EMP-301'`).

- `DISPLAY_AS`: from `pyReferenceComponentType` — `Combobox` → `autocomplete`, `Table` → `table`
- `LABEL`: resolved from the view label properties
- `{ searchFor, searchBy }`: destructured object with optional properties from input parameters `{Label}_searchFor` and `{Label}_searchBy`. Pass `{}` when picker has single category and single group
...
If `displayAs` is `advancedSearch`, use `Handle_SearchAndSelectMulti` from `business-action-advanced-search`; for all other display types, keep using the existing `Handle_ReferenceListMethods` pattern above.
