...
When `pyBaseURLSelectionType` is `"SETTING"`, the URL comes from an Application
Setting (`Rule-Admin-System-Settings`). **Do not confuse these with Dynamic System
Settings (`Data-Admin-System-Settings` / DSS) — they are different rule types.**
Application Settings are integration-focused and are the correct type for REST
connector URL configuration.

Omit `pyBaseURL` and `pyEndpointURL`, and instead set `pyBaseURLSetting` to the
Application Setting reference (e.g.,
...
When `pyBaseURLSelectionType: "SETTING"` (or any field referencing an Application
Setting instance via `pyAuthenticationProfileForSetting` or similar), the referenced
setting (`Rule-Admin-System-Settings`) must already exist in the target ruleset or
its prerequisites. `create-rule` validates references at create time and rejects
...
- Pre-create the referenced Application Settings before submitting the connector, or
...
(`pyAuthenticationProfileForSetting`), and any other Application Setting reference field.
...
- Use `pyBaseURLSelectionType: "SETTING"` with a `pyBaseURLSetting` reference
  whose value differs per environment via Application Settings (`Rule-Admin-System-Settings`).
