...
- **`"SETTING"`** -- when using an application-setting-based auth profile via
...
- **SETTING profile** (`pyAuthProfileSelectionType: "SETTING"`): Set
  `pyAuthenticationProfileForSetting` to the application setting reference (e.g.,
  `"MyOrg-MyApp!pyAuthProfileReference"`). The referenced setting (`Rule-Admin-System-Settings`)
  must pre-exist -- see `rest-url-configuration` ("SETTING references must
