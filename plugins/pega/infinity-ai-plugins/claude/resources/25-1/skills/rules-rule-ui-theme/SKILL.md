...
2. If you are customizing the OOTB class in a branch, call `update-rule` with
   `changeRequestID`; if a branch copy does not exist yet, it is created/used
   automatically.
3. If the branch copy already exists, `update-rule` uses that branch instance.
...
- Confirm the resolved rule is in the active branch ruleset rather than assuming `Pega-UIEngine`.
