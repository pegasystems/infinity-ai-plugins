...
the end. Each rule write call (`create-rule` or `update-rule`; `copy-rule` only for
explicit Save As scenarios) should be followed by updating these fields on the case.
...
| Field | Description |
|-------|-------------|
| `changeRequestID` | The ChangeRequest case ID (e.g., `PEGAACCEL PXC-11`). Pass to all subsequent `create-rule` and `update-rule` calls (`copy-rule` only when an explicit Save As is requested). |
...
copy internally before applying updates.
...
**Never guess or improvise the test ruleset name from the application's main
branch ruleset naming pattern** (e.g. do NOT assume it is
`"{App}_Branch_{BranchName}"` just because that happens to be the main
branch ruleset's pattern). The test ruleset is a **separately named**
ruleset (e.g. `OnboardingTests`) and must always be read from the
`pyGetAppTestRuleSets` field on the `get-assignment` response, never
assumed.
