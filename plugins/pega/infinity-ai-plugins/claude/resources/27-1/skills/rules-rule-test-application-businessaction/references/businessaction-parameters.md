...
**Date format:** Always `YYYY-MM-DD` with dashes. Never slashes.

**DateTime format:** Format depends on the view field's `clockFormat` configuration:

| Clock format | Pattern | Example |
|--------------|---------|---------|
| `12` (default) | `YYYY-MM-DD HH:MM:SS AM/PM` | `2026-07-20 09:00:00 AM` |
| `24` | `YYYY-MM-DD HH:MM:SS` | `2026-07-20 14:30:00` |

Extract `clockFormat` from the view field's `config.clockFormat` (in `pxViewMetadata`) or
`pyClockFormat` (in `pyContent`). When absent or empty, default to 12-hour format.

See `business-action-datetime-12hr` for the 12-hour pattern and `business-action-datetime-24hr` for the 24-hour pattern.
