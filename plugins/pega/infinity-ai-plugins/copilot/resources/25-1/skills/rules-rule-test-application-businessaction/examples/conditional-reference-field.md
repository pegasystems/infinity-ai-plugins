...
```json
{
  ...
  "pyPlaywrightScript": "await caseUtils.openCaseWideActions(page, \"Reassign expense\");\n\nif (params[\"Reassignment reason\"]) { await page.locator(\"//fieldset[@data-testid='Reassignment reason']//label[text()='\" + params[\"Reassignment reason\"] + \"']\").click(); }\nif (params[\"Department\"] &&  params[\"Reassignment reason\"] == \"Transfer to another department\") { await commonUtils.Handle_ReferenceListMethods(page, 'autocomplete', 'Department', 'singleselect', params[\"Department\"]); }\n\nawait caseUtils.clickSubmit(page);"
}
```
