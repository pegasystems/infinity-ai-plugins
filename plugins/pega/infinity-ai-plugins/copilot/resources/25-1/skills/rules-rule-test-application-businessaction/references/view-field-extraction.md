...
- `config.clockFormat` — for DateTime fields: `"24"` = 24-hour format (see `business-action-datetime-24hr`), `"12"` or absent = 12-hour format with AM/PM (see `business-action-datetime-12hr`).
...
- `pyClockFormat` — for DateTime fields: `"24"` = 24-hour format (see `business-action-datetime-24hr`), `"12"` or absent = 12-hour format with AM/PM (see `business-action-datetime-12hr`)
...
- **Fetch the `AdvancedSearch` sub-view via get-rule(detail="full") — MANDATORY for every advancedSearch field.** Never skip this call or assume the group structure. Extract:
  - **ALL "Search for" categories** — from `DeferLoad` children in `SearchCategories` region if present.
  ...
  - Use empty strings for `searchFor`/`searchBy` when only one option exists at that level.
  - The SimpleTableSelect result columns are display-only — exclude from pyInputParameters and pyForm.
  - This data drives the `{Label}_searchFor` and `{Label}_searchBy` input params and the branching structure in `pyPlaywrightScript` (see `business-action-advanced-search`).
...
| Component type | `TextInput`, `Dropdown`, `Checkbox`, `DateTime`, etc. |
...
| Clock format | For DateTime: `24` or `12` (from `config.clockFormat` / `pyClockFormat`; default `12`) |
...
| Search for | Search by | Filter field label | Type | Property ref |
|---|---|---|---|---|

Enumerate **every** category and group from the picker `get-rule(detail="full")`.
Recording only one group **fails the checkpoint**. Pass `{}` for a single-category, single-group picker.

> **DeferLoad categories must all be fetched before filling this table.**
> Each DeferLoad child in `SearchCategories` is a separate view. Call `get-rule(detail="full")` on
> every one before filling any row of this table. An unfetched category = a blank row = wrong
> branching code. Categories can have zero searchBy groups, one, or many — verify each individually.

This table drives the per-group Playwright `if/else if` blocks and the `{Label}_searchFor` /
`{Label}_searchBy` input params (see `business-action-advanced-search`).