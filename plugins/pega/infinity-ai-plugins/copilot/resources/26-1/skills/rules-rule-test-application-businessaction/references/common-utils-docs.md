...
export class CommonUtils {
     ...
     * @param searchFor (optional) value to select in the "Search for" radio/dropdown — omit if not needed
     * @param searchBy (optional) value to select in the "Search by" dropdown — omit if not needed
     * @param searchFields is the array of fields in advancedSearch
     * @param searchPick is the value to select from results ("Name1:ID1")  //Single select
     *
     * @example
     * await commonUtils.Handle_SearchAndSelectSingle(page, 'Customer', { searchFor: 'Customer information', searchBy: 'Last name' },
     *   [{label: 'Last name', type: 'TextInput', value: 'Smith'}], 'John Smith:EMP-301');
     */
    Handle_SearchAndSelectSingle = async (page: Page, label: string, { searchFor, searchBy }: { searchFor?: string; searchBy?: string }, searchFields: any, searchPick: string): Promise<void>;
     ...
     * @param searchBy (optional) value to select in the "Search by" dropdown — omit if not needed
     * @param searchFields is the array of fields in advancedSearch
     * @param searchPicks are the values to select from results ["Name1", "Name2"] //Multi select
     *
     * @example
     * await commonUtils.Handle_SearchAndSelectMulti(page, 'Affected Station', { searchFor: 'Affected Station', searchBy: 'Label' },
     *   [{label: 'Label', type: 'TextInput', value: 't'}], ["Station1", "Station2"]);
     */
    Handle_SearchAndSelectMulti = async (page: Page, label: string, { searchFor, searchBy }: { searchFor?: string; searchBy?: string }, searchFields: any, searchPicks: string[]): Promise<void>;
