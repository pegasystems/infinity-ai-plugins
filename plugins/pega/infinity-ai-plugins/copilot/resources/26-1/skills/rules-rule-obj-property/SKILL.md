...
### Paragraph properties require clarification before authoring

When the user asks to create a property of type **paragraph** (or any long-form text field
intended to display multi-line content), the agent **must ask** which rendering mode is
wanted before calling `create-rule`:

> "Should this paragraph be displayed as **plain text** (no formatting) or **rich text**
> (bold, italic, bullet lists, etc.)?"

Map the answer to `pyStreamName`:

| User answer | `pyStreamName` value |
|-------------|----------------------|
| Plain text  | `pxTextArea`         |
| Rich text   | `pxRichTextEditor`   |

Do not default silently to either option — the choice affects data storage and rendering
and cannot be changed after the property is created (see "Property type is immutable after
creation" below).
