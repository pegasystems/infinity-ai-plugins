...
2. **For rule-level asks that include PegaUnit generation, fetch existing linked PegaUnit tests first** using `run-data-page` on `D_pxGetRuleTestCases` with `dataPageType="list"` and payload:
   - `dataViewParameters.RecordsUnderTest` = rule `pxInsName` in uppercase
   - `dataViewParameters.RecordsUnderTestType` = rule type class (for example `Rule-Obj-When`, `Rule-Obj-Activity`)
...
| `testType` | Yes | `PegaUnit`, `ApplicationTest`, or mixed |
