...
1. **One perform step per Business Action.** Exactly one `PerformAssignment`, `PerformAction`, or `TriggerOptionalProcess` (PerformAssignment may be preceded by `CreateCase` or `CaptureChildCase`). Never chain multiple.
...
7. **TriggerOptionalProcess BAs carry NO form fields.** `pyForm` is empty. `pyInputParameters` contains only `CaseID` (no `ValidationFails`). `ProcessID` is the `pyFlowName` of the optional process from the case type rule — always a Constant. Playwright is a single `openCaseWideActions(page, "PROCESS_LABEL")` call where `PROCESS_LABEL` = `pyLabel` of the optional process (not `pyFlowName`) — no `clickSubmit`, no field interactions. The resulting assignment requires a separate PerformAssignment BA.
...
| Optional/ad-hoc process (`pyOptionalProcesses` or `pyCaseOptionalProcesses`) | TriggerOptionalProcess |
...
| Optional process trigger | `Trigger{ProcessNameNoSpaces}Process` | `TriggerEscalateToManagementProcess` |
...
- `pyPlaywrightScript` is raw TypeScript body statements — no markdown fences or language tags.
...
NEVER modify, update, or copy any of the following Business Actions. They are pre-provisioned and must be used exactly as-is:
- **Foundation BAs (Work-):** Login, GetCase, AssertCase, AssertCaseField, AssertCaseStage, AssertCaseStatus, AssertError, NotFound, IgnoreDuplicateCase, ResolveasDuplicateCase
- **Customer service application specific foundation BAs (PegaCS-Work-Interaction):** CreateInteractionCaseForVerifiedCustomer, CreateInteractionUnknownCustomer, CreateInteractionCaseForNonVerifiedCustomer, VerifyCustomer, AddServiceCase, WrapUpInteraction — these are only present if the application is built on the Customer Service application.

Only create NEW Business Actions for assignments not already covered by the above.

When the user explicitly asks to migrate these read-only Business Actions, apply these changes to `pyPlaywrightScript` only:
1. **Remove the async wrapper.** Keep only the body statements; strip the function declaration, parameter list, and closing brace. Leave the legacy `pyPlaywrightMethod` field unchanged; Pega no longer reads it.
2. **Replace bare parameter variables with `params["..."]`.** Every bare variable that corresponds to an input parameter becomes `params["<exact pyParameterName>"]`.
3. Do not change `pyTestSteps`, `pyForm`, `pyInputParameters`, `pyActionParameters`, or any other fields.

> **Legacy migration note:** Existing Business Actions may still contain `pyPlaywrightMethod` because `update-rule` deep-merges updates with the fetched rule. Pega no longer reads this field during Business Action processing, so migration must not be blocked by its presence. Update `pyPlaywrightScript` only; do not attempt to delete or rewrite `pyPlaywrightMethod` unless explicitly required by the server.
