...
3. For each Business Action returned, call `get-rule(key="{insKey}", detail="full")` to fetch `pyLabel`, `pyInputParameters` and `pyOutputParameters`.
...
- `pyLabel`: the Business Action's `pyLabel` from inventory — **MUST exactly match** the label of the existing Business Action rule. The platform validates this field against existing Business Action labels; a mismatch causes a "Label must match an existing Business Action label" error.
...
- **TriggerOptionalProcess Business Actions**: wire only `CaseID` from `Keyword Output`. No `ValidationFails`, no form field parameters.
