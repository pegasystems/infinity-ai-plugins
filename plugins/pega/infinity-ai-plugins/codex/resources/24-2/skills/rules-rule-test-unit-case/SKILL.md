...
- **Connector-backed Data Page assertions.** `ResultCount` assertions work
  ...
  are fragile against connector-backed sources — an assertion structured
  identically to a working local-Data-Page pattern can fail even when the
  underlying data is independently confirmed correct via `run-data-page`.
  Root cause not fully determined. Prefer a `ResultCount` assertion on the
