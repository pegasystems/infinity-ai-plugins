...
> **Usage context:** Loaded by `methodology-application-tests` (Phase 4) before generating Gherkin scenarios. Can also be loaded standalone. When loaded standalone, use the Example Reference Skills table in `methodology-application-tests` to choose scenario example skills.
