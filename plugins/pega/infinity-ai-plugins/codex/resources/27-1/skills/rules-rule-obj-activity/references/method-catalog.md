...
| Keyword | Description | Skill |
|---------|-------------|--------------|
| `Call` | Call another activity (same-class or cross-class via `Call Class.Activity`) | `Call (Activity name)` |
| `Branch` | Transfer control to another activity without returning | `Branch (Activity name)` |
| `Collect` | Collect pages from child requestors | — |
| `Java` | Execute inline Java code | `Java` |
...
| `Save-DataPage` | Save-DataPage | Persists a savable Data Page's save plan. Params: `DataPage`, key property (e.g. `pyGUID`), `WriteNow: "true"`. Only valid against a savable (`pyPageType: "savable"`) Data Page. |
